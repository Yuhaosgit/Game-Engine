...
std::vector<std::shared_ptr<Terrain>> Terrain::terrains;
...
Terrain::Terrain(const std::string& name, const float& stretchFlat, const float& stretchHeight, const float& texScale) {
	int channels;
	auto path = Importer::FindFile(name, "../Terrain");
...
float Terrain::GetHeight(const unsigned int& xPos, const unsigned int& zPos) const {
	int x = 0, z = 0;
	...
	return mesh.lock()->vertices[index].y;
}