	...
	if (!FBXfiles.empty()) {
		...
		if (scene->HasMeshes()) {
			for (int i = 0; i < scene->mNumMeshes; ++i) {
				...
				model->mesh = LoadMesh(scene->mMeshes[i]);
			}
		}
		auto test = scene->mMaterials;
		auto test2 = test[0];
		aiString nn;
		auto t = test2->GetTexture(aiTextureType::aiTextureType_NORMALS, 0, &nn);
	}
}
...
void Importer::LoadShader(const std::string& name, const std::string& vertex, const std::string& fragment,
    ...
    if (ShaderFiles.find(vertex) == ShaderFiles.end())
        ShaderLoader::LoadShader(vertex);
    if (ShaderFiles.find(fragment) == ShaderFiles.end())
        ShaderLoader::LoadShader(fragment);
    if (ShaderFiles.find(geometry) == ShaderFiles.end())
        ShaderLoader::LoadShader(geometry);
    if (ShaderFiles.find(domain) == ShaderFiles.end())
        ShaderLoader::LoadShader(domain);
    if (ShaderFiles.find(hull) == ShaderFiles.end())
        ShaderLoader::LoadShader(hull);

    ShaderFiles.insert(vertex);
    ShaderFiles.insert(fragment);
    ShaderFiles.insert(geometry);
    ShaderFiles.insert(domain);
    ShaderFiles.insert(hull);
...
Material* Importer::ConstructMaterial(std::vector<std::string>& textures, Shader* shader) {
	...
	for (auto& texture : textures) {
		...
		if (texture.find("color") != std::string::npos || texture.find("albedo") != std::string::npos) {
			material->SetTexture2D("diffuseTex", Importer::TextureSet[texture]);
		}
		else if (texture.find("normal") != std::string::npos) {
			material->SetTexture2D("normalTex", Importer::TextureSet[texture]);
		}
		else if (texture.find("npecular") != std::string::npos) {
			material->SetTexture2D("specularTex", Importer::TextureSet[texture]);
		}
		else if (texture.find("gloss") != std::string::npos) {
			material->SetTexture2D("glossTex", Importer::TextureSet[texture]);
		}
	}
	return material;
}
